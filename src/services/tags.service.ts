...
class TagsService extends ResolversOperationsService {
    collectionGenre = COLLECTIONS.TAGS;
    ...
    async items(active: string = ACTIVE_VALUES_FILTER.ACTIVE) {
        ...
        const result = await this.list(this.collectionGenre, 'tags', page, itemsPage, filter);
    ...
    async insert() {
        ...
        const result = await this.add(this.collectionGenre, tagObject, 'tag');
        return { status: result.status, message: result.message, tag: result.item };
    }
    async modify() {
        ...
        const result = await this.update(this.collectionGenre, { id }, objectUpdate, 'genero');
        return { status: result.status, message: result.message, tag: result.item };
    }
    ...
    async delete() {
        ...
        const result = await this.del(this.collectionGenre, { id }, 'genero');
    ...
    async unblock(unblock: boolean = false) {
        ...
        const result = await this.update(this.collectionGenre, { id }, { active: unblock }, 'tag');
